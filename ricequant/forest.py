...
def init(context):
    context.counter = 0
    context.period = 800
    context.observation = 1000
    context.clf = RandomForestClassifier()
...
def predict(x, context):
    pred = context.clf.predict(x)
    
    return pred
    
def predict_prob(x, context):
    prob = context.clf.predict_proba(x)
    return prob
...
def handle_bar(context, bar_dict):
    ...
    # plot('prob', up_prob)
    # plot('pred', pred)
    # print(context.clf.feature_importances_)
    # print(prob)
    # print(pred)
    
    if up_prob > 0.9:
        ret = order_target_percent(context.s1, 0.99)
        if ret != None:
